...
def validate_mask_confirmation(paths: RunPaths) -> MaskConfirmationGate:
    paths = _coerce_paths(paths)
    blockers: list[str] = []
    receipt_path = _receipt_path(paths)
    try:
        ...
        draft_record = bundle.get("assets", {}).get("draft_assessment_path", {})
        draft_path = _resolve_record_path(paths.root, draft_record.get("path"))
        assessment = _load_json(draft_path, "draft Mask Assessment")
        if assessment.get("status") != "review" or assessment.get(
            "technical_blockers"
        ) != []:
            blockers.append("technical_gate_failed")
