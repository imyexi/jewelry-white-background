...
**技术栈：** Python 3、标准库、Pillow、unittest、AIReiter 既有 helper、Yuan Studio 既有水印脚本。

## 全局约束

- 所有用户可见文档与提示词使用中文。
- `3:4`、`2K` 仅作为 AIReiter 参数，不写入 V2 提示词。
- 不自动裁切、缩放或重排生成图。
...
- [x] 从既有 SY1462 来源图生成三次本地 V2 金丝雀；均使用 AIReiter 参数传递画幅和分辨率，未向飞书写回。
